...
function [daughter,fourier_factor,coi,dofmin] = ...
	wave_bases(mother,k,scale,param,dt);
...
%    norm = sqrt(2*pi*scale/dt)*(pi^(-0.25));            % modified by Yovanni   [Eqn(7)]
...
% 	norm = sqrt(scale*k(2))*(2^m/sqrt(m*prod(2:(2*m-1))))*sqrt(n);
    norm = sqrt(2*pi*scale/dt)*(2^m/sqrt(m*prod(2:(2*m-1))));
...
% 	norm = sqrt(scale*k(2)/gamma(m+0.5))*sqrt(n);
    norm = sqrt(2*pi*scale/dt/gamma(m+0.5));
