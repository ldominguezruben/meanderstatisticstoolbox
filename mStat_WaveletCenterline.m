...
% %Getting the Fourier peaks
% lenWS = length(global_ws);
% xposFM = 0.0; xposFM=0.0; PrintFPeak=0;
% for rr=1:lenWS
%     if (global_ws(rr)<global_signif(rr))
%        % if (global_ws(rr)>xposFM) 
%             PrintFPeak=1;
%             xposFM=global_ws(rr)*DeltaCentS*DeltaCentS;
%             yposFM=log2(period(rr)*DeltaCentS);
%         %end
%     else
%         if (PrintFPeak==1)
%             StrPeak = num2str(2^yposFM,'%6.2f');
